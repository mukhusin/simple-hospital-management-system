...
@csrf
								...
								<tr>
									<td>
										<select class="chosen-select-width" name="med[]" data-placeholder="Select Final Diasnosis " >
										    @foreach(Widget::meds_list('',$attend->insurance_id) as $k => $v)
										        <option value="{{$k}}"
						...
						@if ($i == 0)
							@for ($i = 0; $i < 3; $i++)
								<tr>
									<td>
										{{Form::select('med[]',
											Widget::meds_list('',$attend->insurance_id),
											'',
			...
			<div class="pane1 btn-warning">
				<h3>Procedures</h3>
				
				<select class="chosen-select-width" name="procedures[]" data-placeholder="Select Procedures" multiple>
				    @foreach(Widget::procedures_list() as $k => $v)
				        <option value="{{$k}}">
				        	{{$v}}
				        </option>
				    @endforeach
			...
			<div class="pane1 btn-primary">
				<h3>Service</h3>
				<div class="form-inline">
					<div class="form-group">
						<label>Service Description</label>
						<input type="text" class="form-control"  name="service_remark" value="" >
					</div>

					<div class="form-group">
						<label>Service Amount</label>
						<input type="text" class="autoNumber form-control" data-v-min="0" name="service_amount" value="" >
					</div>
				</div>
			...
			<div class="pane1 well1">

			<h3>Patient Type, Remark & Followup</h3>
			<div class="col-lg-12 well">
				<div class="col-lg-3">
					<label>Patient Type </label>
					<?php $temp1 = array('Out','In'); ?>
					{{Form::select('type',array('Out'=>'Out Patient','In'=>'In Patient'),$temp1[$attend->ward_served],array('class' => 'form-control ward_trig'))}}
				</div>
				<div class="col-lg-3">
			...
			<div class="form-group">
				<label>Remark</label>
				<textarea name="remark" class="text_editor">{{$attend->remark}}</textarea>
			</div>
			...
			<div class="form-inline">
				<div class="form-group">
					...
					<input type="date" name="followup" value="" >
				</div>
				<div class="form-group">
					<label>Follow Time: </label>
					<input type="time" name="followup_time" value="" >
				</div>
			</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".ward_trig").live('change', function() {
			if (this.value == 'In') {
				// $(".ward_trig");
			};
			// $(".ward_trig").
