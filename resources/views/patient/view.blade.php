...
@section('main')
                ...
                @elseif (Auth::user()->office_id == 6 && $patient->is_new())
                    ...
                    @elseif ((Auth::user()->isDoctorOffice() && $patient->is_inoffice()) || $patient->is_active_inpatient())
                        <div class="btn-group btn-group-justified">
                            ...
                            <a href="{{ url('patient/edit/vital/' . $attend->patient->id) }}"
                                class="btn btn-info col-lg-3 big-btn">
                                <i class="icon-stethoscope"></i>
                                Vital Sign
                            </a>

                            <a href="{{ url('patient/edit/allegies/' . $attend->patient->id) }}"
                                class="btn btn-info col-lg-3 big-btn">
                                <i class="icon-stethoscope"></i>
                                Allegies
                            </a>

                            <a href="{{ url('attendance/clinical/' . $attend->id) }}"
                                class="btn btn-success col-lg-3 big-btn">
                                <i class="icon-stethoscope"></i>
                                Clinical Information
                            </a>

                            <a href="{{ url('attendance/investigation/' . $attend->id) }}"
                                class="btn btn-primary col-lg-3 big-btn">
                                <i class="icon-code"></i>
                                Investigation
                            </a>
                        ...
                        @elseif (Auth::user()->office_id == 4 && $patient->is_inoffice())
                            ...
                            @elseif (Auth::user()->office_id == 5 && $patient->is_inoffice())
                                <div class="btn-group btn-group-justified">
                                    ...
                                    <a href="{{ url('attendance/dispense/' . $attend->id) }}"
                                        class="pause_attendance btn btn-info col-lg-3 big-btn">
                                        <i class="icon-signin"></i>
                                        Drug Dispense
                                    </a>
                                    <a href="{{ url('patient/return/action/' . $attend->id) }}"
                                        class="verify btn btn-danger col-lg-3 big-btn">
                                        <i class="icon-medkit"></i>
                                        Return to Doctor
                                    </a>
                                @elseif (Auth::user()->office_id == 6 && $patient->is_inoffice())
                                    <div class="btn-group btn-group-justified">
                                        ...
                                        <a href="{{ url('attendance/payment/' . $attend->id) }}"
                                            class="btn btn-info col-lg-3 big-btn">
                                    ...
                                    @else
                                        <div class="btn-group btn-group-justified">
                                            <div class="bg-danger" style="border-radius:5px">
                                                <p style="padding:20px">You have no official access to this patient yet!
                                                    (She/He is not supposed to be in your office)</p>
                                            </div>
            @endif
                        ...
                        <div class="well well1 overflow">
                            ...
                            @if ($a->doctor_id)
                                ...
                                <div class="form-group2">
                                    ...
                                    <div>
                                        {{ date('H:i D, d M Y', strtotime($a->created_at)) }}
                                    </div>
                                </div>
                                ...
                                @foreach ($a->clinicals as $tem_key => $c)
                                    <div class="block_in">
                                        <h5>
                                            Record updated {{ date('H:i D, d M', strtotime($c->created_at)) }} by
                                            {{ ucwords($c->creator->name) }}

                                            <a style="display: block;" class="text-right vhidden"
                                                href="{{ url('attendance/clinical/edit2', $c->id) }}"> more </a>
                                            <hr class="">
                                        </h5>
                                        ...
                                        @if (count($lr) > 0)
                                            ...
                                            <table class="table well well3">
                                                ...
                                                @foreach ($lr as $t)
                                                    <tr>
                                                        <td>{{ LabTest::find($t->test_id)->code }} :
                                                            {{ LabTest::find($t->test_id)->name }}</td>
                                                        <td>{{ $t->results_register }}</td>
                                                        <td>
                                                            @if ($t->attachment)
                                                                <a href="{{ url($t->attachment) }}"
                                        ...
                                        @if (count($me) > 0)
                                            ...
                                            <table class="table well well2">
                                                ...
                                                @foreach ($me as $t)
                                                    <tr>
                                                        <td>{{ Medical::find($t->medical_id)->name }} -
                                                            {{ Medical::find($t->medical_id)->brand }}</td>
                                                        <td>{{ $t->dosage }}</td>
    ...
    <div class="tab-pane" id="invest">
        <div class="well well1">
            <h3>Patient Investigation Results
                <hr>
            </h3>
            @if (isset($attend))
                <?php $in = $attend->labResults;
                $temp = 0; ?>
                @foreach ($in as $t)
                    <?php ++$temp; ?>
                    <div class="form-group">
                        ...
                        @foreach ($ab as $b)
                            @if ($b->checkout)
                                <tr>
                                    ...
                                    <td>{{ number_format(($b->ward->price * (strtotime($b->checkout) - strtotime($b->checkin))) / (60 * 60), 2) }}
                                    </td>
                ...
                @if (Auth::user()->role_id == 3)
                    <div class="col-lg-12 well">
                        <form method="POST" action="{{ url()->current() }}">
                            @csrf
                            <div class="col-lg-12">
                                <h4>Change Ward / Bed (This will have impact on Bill)
                                    <hr />
                                </h4>
                            </div>
                            <div class="col-lg-3">
                                ...
                                <?php $temp1 = ['Out', 'In']; ?>
                                <input type="text" name="bed" class="form-control" readonly="readonly"
                                    value="{{ $temp1[$attend->ward_served] }}">
                            </div>
                            <div class="col-lg-3">
                                <label>Ward Number </label>
                                {{ Form::select('ward', Widget::wards(), $attend->ward_id, ['class' => 'form-control ward_served']) }}
                            </div>
                            <div class="col-lg-3">
                                <label>Patient Bed Number / Tag</label>
                                <input type="text" name="bed" class="form-control ward_served"
                                    value="{{ $attend->bed }}">
                            </div>
                            <div class="col-lg-3">
                                <label>&nbsp;</label>
                                <input type="hidden" name="section" value="ward">
                                <input type="submit" name="button" class="form-control btn btn-danger"
                                    value="Save Changes">
                            </div>
                        </form>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        {{ Form::select('office', Widget::offices(), '', [
                            'class' => 'form-control',
                            'required',
                        ]) }}
                        <!-- <hr>
                    ...
                    <div class="modal-body">
                        ...
                        @if ($patient->insurance)
                            <div>
                                ...
                                {{ Form::select('payment', Widget::paymentMethod($patient->insurance), '', [
                                    'class' => 'form-control',
                                    'required',
                                ]) }}
                            </div>
                        ...
                        <div>
                            ...
                            {{ Form::select('office', Widget::offices(1), '', [
                                'class' => 'form-control',
                                'required',
                            ]) }}
                        </div>
